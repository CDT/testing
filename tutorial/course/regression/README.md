# Chapter 07: regression solution
...
node --test tests/regression/checkout.test.js
```

Expected: 16 API tests total (6 integration, 7 security, 3 regression). The fixture starts real HTTP servers on ephemeral loopback ports and cleans up after every test. No running app is required.
...
The included .github/workflows/testing.yml assumes demo-store is your Git repository root. Commit it there to enable PR/push deterministic and browser checks. Its performance job only runs on workflow_dispatch. In the parent course repository, use its separate root workflow, which sets working-directory and cache paths for the nested app. Neither workflow deploys.

Final counts: 28 unit + 6 integration + 7 security API + 3 regression = 44 deterministic cases; 6 browser cases; four performance profiles. The three REG labels identify course scenarios, not claimed historical incidents.
