...
Expected: 5 browser passes, then 2 smoke passes. The config builds and starts the real app on port 5175 and stops it afterward. Leave that port free. It uses one worker, zero retries, fresh browser contexts, and a fresh server per invocation. Sessions are isolated; inventory is shared within the invocation. Do not increase workers or retries without redesigning inventory fixtures.
